...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($username) || empty($password)) {
        ...
        if ($user && password_verify($password, $user['password'])) {
            // Set session (Cookie + Native)
            setcookie('user_id', $user['id'], time() + 86400, '/', '', true, true);
            setcookie('username', $user['username'], time() + 86400, '/', '', true, true);
            setcookie('nama', $user['nama'], time() + 86400, '/', '', true, true);
            setcookie('level', $user['level'], time() + 86400, '/', '', true, true);
